---
title: "Analyse factorielle multiple sur des oiseaux"
author: "___"
date: "`r Sys.Date()`"
format:
  html:
...
Les massifs montagneux des Alpes françaises (**French alps**) offrent une diversité d'habitats pour les oiseaux, favorisant ainsi une grande variété d'espèces. La fréquentation humaine, variable selon le relief et l'accessibilité des lieux, joue un rôle important dans la répartition de ces espèces. En effet, les zones plus difficiles d'accès abritent souvent des oiseaux plus farouches qui ne supportent pas la proximité de l'homme.

Afin de mieux comprendre la répartition des oiseaux dans cet environnement complexe, des scientifiques ont étudié 19 espèces dans 23 régions des Alpes françaises. Ils ont également relevé les températures et les précipitations dans chaque station pour déterminer l'influence des facteurs environnementaux sur la composition de la faune aviaire.

L'objectif de cette étude est d'identifier les stations et/ou les espèces qui présentent des particularités par rapport aux autres.
...
<!--% Importez le jeu de données `meteo` se trouvant dans le dossier `data/` sous le nom de `alps_meteo.csv`. Décrivez-les avec skimr::skim(). -->
...
select_answer("
[] -  Ce tableau ne contient aucune valeur manquante.
...
Le tableau et le graphique ci-dessous illustrent les données récoltées lors de l'observation de 19 espèces d'oiseaux dans 23 stations distinctes. Les niveaux d'abondance observés ont été regroupés en 4 catégories :
...
<!--% Importez le jeu de données `birds` se trouvant dans le dossier `data/` sous le nom de `alps_birds.csv`. Décrivez-les avec skimr::skim(). -->
...
<!--% Visualisez ces données sur un graphique de type raster en utilisant geom_raster(). Les labels a proposer sont les suivants Espèce, Station, Abondance. -->
...
<!--# Comme ce graphique est identique à celui traité dans le module précédent, l’interprétation est déjà réalisée pour vous.. -->

Dans notre jeu de données, nous avons 19 variables qualitatives ordonnées. Ce jeu de données est complet, sans aucune valeur manquante. De plus, les données sont déjà encodées de manière appropriée, ce qui signifie qu'elles ne nécessitent pas de transformation supplémentaire. En examinant les stations, nous constatons que les stations 15 à 23 ont tendance à accueillir davantage d'oiseaux. Parmi les espèces, la fauvette mélanocéphale est la moins abondante.
...
<!--% Effectuez une analyse factorielle multiple (AFM) sur le tableau alps et stockez l'objet résultant dans la variable `alps_mfa`. Ensuite, affichez le résumé de l'objet alps_mfa. Lors de l'analyse, traitez les variables de dénombrement comme des variables destinées à l'analyse de fréquence. -->
...
<!--% Réalisez le graphique des éboulis de votre AFM avec données mixtes. -->

```{r screemfa, record='RNP', arg='labels'}

```

<!--% Interprétez le résumé de l'AF et de son graphique des éboulis. -->
...
<!--% Créez un graphique représentant la répartition des variables dans l'espace du premier plan de l'analyse factorielle multiple (AFM). -->

```{r loadingsmfa, record='RNP', arg='labels'}

```

<!--% Interprétez le graphique représentant la répartition des variables dans l'espace. -->

```{r loadingsmfacomment, output='asis'}
select_answer(r"-{
[] -  Toutes les variables sont bien représentées dans le plan de l’AFM. Cela est indiqué par la longueur élevée des vecteurs.
[] -  Toutes les variables sont mal représentées dans le plan de l’AFM. Cela est indiqué par la longueur élevée des vecteurs.
...
<!--% Créez un graphique représentant la répartition des axes dans l'espace du premier plan de l'analyse factorielle multiple (AFM). -->

```{r axesmfa, record='RNP', arg='labels'}

```

<!--% Créez un graphique représentant la répartition des groupes dans l'espace du premier plan de l'analyse factorielle multiple (AFM). -->

```{r groupsmfa, record='RNP', arg='labels'}

```

<!--% Interprétez le graphique représentant la répartition des groupes et des axes dans l'espace. -->
...
<!--% Créez un graphique représentant la répartition des stations et des espèces dans l'espace du premier plan de l'analyse factorielle multiple (AFM). -->

```{r contingencymfa, fig.width=10, record='RNP', arg='labels'}

```

<!--% Interprétez le graphique représentant les stations et les espèces dans le premier plan de l'AFM. -->

```{r contingencymfacomment, output='asis'}
select_answer(r"-{
[] -  Les stations sont très dispersées.
[] -  Les stations sont parfaitement regroupées.
[] -  Les stations sont regroupées en 4 groupes. Le premier groupe est composé de la station 7. On retrouve un second groupe avec la station 16 et 22. Le dernier groupe reprend tous les autres
...
[] -  La station 7 est marquée par des température minimales et mximales plus marquées que dans les autres stations.
}-")
```

# Discussion et conclusions

<!--% En conclusion, l'AFM vous semble-t-elle être un outil adéquat pour atteindre l'objectif de ce travail ? Concluez en deux phrases. Est ce que l'AFM vous donne des résultats plus intéressant que l'AFC ? -->

..Votre discussion ici...

# Référence

<!--% Ne rien indiquer ici. La référence bibliographique se placera automatiquement.-->
